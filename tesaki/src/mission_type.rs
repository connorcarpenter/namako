...
impl MissionType {
    ...
    pub fn category(&self) -> MissionTypeCategory {
        match self {
            ...
            Self::RefineFeatureIntent { .. }
            | Self::AddOrClarifyScenario { .. }
            | Self::DraftSpecScenarios { .. }
            | Self::PromoteScenariosToExecutable { .. } => MissionTypeCategory::Spec,
    ...
    pub fn default_surface_policy(&self) -> SurfacePolicy {
        match self {
            ...
            | Self::AddOrClarifyScenario { .. }
            | Self::DraftSpecScenarios { .. }
            | Self::PromoteScenariosToExecutable { .. } => SurfacePolicy::for_refine_spec(),
    ...
    pub fn expected_evidence_change(&self) -> Vec<EvidenceChange> {
        match self {
            ...
            Self::ImplementBehaviorForScenario { .. }
            | Self::FixRegressionFromGateFailure { .. } => {
...
mod tests {
    ...
    fn recommended_model_sonnet_for_structured_work() {
        ...
        assert_eq!(
            MissionType::StrengthenThenAssertions {
                scenario_key: "s".into(),
                weak_steps: vec![]
            }
            .recommended_model(),
        ...
        assert_eq!(
            MissionType::RefactorBindingsForClarity {
                binding_ids: vec![]
            }
            .recommended_model(),
            "sonnet"
    ...
    fn recommended_model_haiku_for_trivial_tasks() {
        ...
        assert_eq!(
            MissionType::CleanupAfterSuccess.recommended_model(),
            "haiku"
        );
