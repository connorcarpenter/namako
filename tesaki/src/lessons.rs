...
impl LessonsDatabase {
    ...
    pub fn load(spec_root: &Path) -> Result<Self> {
        ...
        let content = fs::read_to_string(&lessons_path).context("Failed to read lessons.json")?;

        let db: LessonsDatabase =
            serde_json::from_str(&content).context("Failed to parse lessons.json")?;
    ...
    pub fn save(&self, spec_root: &Path) -> Result<()> {
        ...
        fs::create_dir_all(&tesaki_dir).context("Failed to create .tesaki directory")?;

        let lessons_path = tesaki_dir.join("lessons.json");
        let content =
            serde_json::to_string_pretty(self).context("Failed to serialize lessons database")?;

        fs::write(&lessons_path, content).context("Failed to write lessons.json")?;
    ...
    pub fn add_attempted_approach(&mut self, issue_key: &str, approach: &str) -> bool {
        if let Some(lesson) = self
            .lessons
            .iter_mut()
            .find(|l| l.issue_key == issue_key && l.resolution.is_none())
        {
...
mod tests {
    ...
    fn test_mark_resolved() {
        ...
        assert_eq!(
            db.lessons[0].resolution,
            Some("Unlocked spec surface and fixed".to_string())
        );
    }
