use std::{borrow::Cow, cell::RefCell, fmt::Debug, mem, sync::LazyLock};

use namako_engine::{Event, Writer, cli, event, given, parser, then, when};
use regex::Regex;
...
mod spec {
    ...
    // TODO: Switch back to `lazy-regex::regex!()` once it migrates to `std`:
    //       https://github.com/Canop/lazy-regex/issues/10
    static FULL_PATH: LazyLock<Regex> =
        LazyLock::new(|| Regex::new(r"((\?|\\|\/).*(\\|\/))?tests((\\|\/)\w*)*").unwrap());
    ...
    #[tokio::test]
    async fn test() {
        let walker = GlobWalkerBuilder::new("tests/features/output", "*.feature")
