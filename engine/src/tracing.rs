...
where
    W: World,
    P: Parser<I>,
    runner::Basic<W, WhichSc>: Runner<W>,
    Wr: Writer<W>,
    Cli: clap::Args,
{
...
impl Collector {
    ...
    pub(crate) fn start_scenarios(
        ...
        for (id, f, r, s, _) in runnable.as_ref() {
            drop(
                self.scenarios
                    .insert(*id, (f.clone(), r.clone(), s.clone())),
            );
        }
    }
...
impl ScenarioId {
    ...
    const SPAN_FIELD_NAME: &'static str = "__namako_scenario_id";
    ...
    pub(crate) fn scenario_span(self) -> Span {
        ...
        // filter to skip it.
        tracing::error_span!("scenario", __namako_scenario_id = self.0)
    }
...
mod suffix {
    ...
    pub(crate) const END: &str = "__namako__scenario";
