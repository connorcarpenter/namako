...
impl<Out: io::Write> Basic<Out> {
    ...
    pub(crate) fn scenario<W: Debug>(
        ...
        match ev {
            ...
            Scenario::Background(bg, ev) => {
                self.background(feat, scenario, bg, ev)?;
            }
            Scenario::Step(st, ev) => {
                self.step(feat, scenario, st, ev)?;
            }
            Scenario::Finished => {
                self.indent = self.indent.saturating_sub(2);
            }
    ...
    pub(crate) fn step<W: Debug>(
        ...
        match ev {
            ...
            Step::Passed(captures, _) => {
                self.step_passed(sc, step, captures)?;
    ...
    pub(crate) fn background<W: Debug>(
        ...
        match ev {
            ...
            Step::Passed(captures, _) => {
                self.bg_step_passed(sc, bg, captures)?;
                self.indent = self.indent.saturating_sub(4);
            }
            Step::Skipped => {
