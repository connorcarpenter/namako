...
/// Writer of [`Namako`] events to some output.
///
/// As [`Runner`] produces events in a [happened-before] order (see
/// [its order guarantees][1]), [`Writer`]s are required to be [`Normalized`].
///
/// As [`Namako::run()`] returns [`Writer`], it can hold some state inside for
/// inspection after execution. See [`Summarize`] and
/// [`Namako::run_and_exit()`] for examples.
///
/// [`Namako`]: crate::event::Namako
/// [`Namako::run()`]: crate::Namako::run
/// [`Namako::run_and_exit()`]: crate::Namako::run_and_exit
/// [`Runner`]: crate::Runner
...
pub trait Writer<World> {
    ...
    /// Handles the given [`Namako`] event.
    ///
    /// [`Namako`]: crate::event::Namako
    fn handle_event(
        &mut self,
...
pub trait Ext: Sized {
    ...
    /// Wraps this [`Writer`] to fail on [`Skipped`] [`Step`]s.
    ///
