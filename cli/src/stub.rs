...
fn generate_single_stub(
    ...
    let orphan = orphans
        ...
        .ok_or_else(|| {
            let available: Vec<_> = orphans.iter().map(|o| &o.binding_id[..16.min(o.binding_id.len())]).collect();
            anyhow::anyhow!(
...
fn generate_stub_feature(orphans: &[OrphanBinding]) -> String {
    ...
    lines.push("# Delete this file after implementing real scenarios for these bindings.".to_string());
