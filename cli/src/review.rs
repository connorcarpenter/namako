...
pub struct PromotionCandidate {
    ...
    /// Blocker classification (from @Blocker tag or default UNKNOWN)
    pub blocker: BlockerType,
    /// Whether this is a stub scenario (always false in promotion_candidates output,
    /// since stubs are filtered out; kept for defense-in-depth in Tesaki)
    #[serde(default)]
    pub is_stub: bool,
}
...
fn analyze_feature(
    ...
    if feature.rules.is_empty() {
        ...
        // Exclude @Stub scenarios per TODO.md §2: stubs must NOT be promotable
        for scenario in &feature.scenarios {
            if is_deferred_scenario(scenario, &lines) && !is_stub_scenario(scenario) {
                ...
                promotion_candidates.push(PromotionCandidate {
                    ...
                    reuse_score,
                    blocker,
                    is_stub: false, // Stubs are filtered; this is for defense-in-depth
                });
            }
        }
...
/// Check if a scenario is a stub (generated by `namako stub`).
///
/// Stub scenarios are hygiene-only placeholders for orphan bindings. They MUST:
/// - NOT appear in promotion candidates
/// - NOT appear in missing bindings worklist
/// - NOT be selected as tasks by Tesaki
///
/// Per TODO.md §2: This prevents autonomy traps where Tesaki might try to
/// "promote" stubs that exist only to satisfy the orphan binding hard error.
fn is_stub_scenario(scenario: &Scenario) -> bool {
    for tag in &scenario.tags {
        // Case-sensitive match on "Stub" (as produced by `namako stub`)
...
"#;
    ...
    #[test]
    fn test_is_stub_scenario_with_stub_tag() {
        // Create a scenario with @Stub tag
        let scenario = Scenario {
            keyword: "Scenario".to_string(),
            name: "Stub for orphan".to_string(),
            description: None,
            steps: vec![],
            examples: vec![],
            tags: vec!["Deferred".to_string(), "Stub".to_string()],
    ...
    #[test]
    fn test_is_stub_scenario_with_at_stub_tag() {
        // Create a scenario with @Stub tag (with @ prefix)
        let scenario = Scenario {
            keyword: "Scenario".to_string(),
            name: "Stub for orphan".to_string(),
            description: None,
    ...
    #[test]
    fn test_is_stub_scenario_without_stub_tag() {
        // Create a regular deferred scenario without @Stub
        let scenario = Scenario {
            keyword: "Scenario".to_string(),
            name: "Real deferred scenario".to_string(),
            description: None,
            steps: vec![],
            examples: vec![],
            tags: vec!["Deferred".to_string()],
            span: Default::default(),
            position: gherkin::LineCol { line: 1, col: 1 },
        };
    ...
    #[test]
    fn test_stub_scenarios_excluded_from_promotion_candidates() {
        // This test verifies the core requirement from TODO.md §2:
        // Stub scenarios must NEVER appear in promotion candidates.
        //
        // The test creates synthetic scenarios and verifies the filtering logic.
        let stub_scenario = Scenario {
            keyword: "Scenario".to_string(),
            name: "Stub scenario".to_string(),
            description: None,
            steps: vec![],
            examples: vec![],
            tags: vec!["Deferred".to_string(), "Stub".to_string()],
        ...
        let real_deferred = Scenario {
            keyword: "Scenario".to_string(),
            name: "Real deferred".to_string(),
            description: None,
            steps: vec![],
            examples: vec![],
            tags: vec!["Deferred".to_string()],
            span: Default::default(),
            position: gherkin::LineCol { line: 10, col: 1 },
        };

        let lines: Vec<&str> = vec![];

        // Stub: is deferred but IS stub -> should be excluded
        assert!(is_deferred_scenario(&stub_scenario, &lines));
        assert!(is_stub_scenario(&stub_scenario));

        // Real: is deferred and NOT stub -> should be included
        assert!(is_deferred_scenario(&real_deferred, &lines));
        assert!(!is_stub_scenario(&real_deferred));

        // The filter logic: `is_deferred && !is_stub`
        // Stub scenario: true && !true = false (excluded)
        // Real deferred: true && !false = true (included)
    }
}
