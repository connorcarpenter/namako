...
pub struct Libtest<W, Out: io::Write = io::Stdout> {
    ...
    /// [1]: https://doc.rust-lang.org/rustc/tests/index.html
    events: Vec<parser::Result<Event<event::Namako<W>>>>,
...
impl<W: World + Debug, Out: io::Write> Writer<W> for Libtest<W, Out> {
    ...
    async fn handle_event(
        ...
        self.handle_namako_event(event, cli);
    }
}
...
impl<W: Debug + World, Out: io::Write> Libtest<W, Out> {
    ...
    /// Handles the provided [`event::Namako`].
    ///
    ...
    /// Converts the provided [`event::Namako`] into [`LibTestJsonEvent`]s.
    fn expand_namako_event(
        &mut self,
        event: parser::Result<Event<event::Namako<W>>>,
        cli: &Cli,
        ...
        match event.map(Event::split) {
            ...
            Ok((Namako::Feature(feature, ev), meta)) => {
